...
#define TNN_LOAD_FUNCTION_PTR(func_name)                                                                    \
    ...
    {                                                                                                       \
        std::cout << "load func (" << #func_name << ") from (" << library_path << ") failed!" << std::endl; \
        return false;                                                                                       \
    }
...
cl_int clGetPlatformIDs(cl_uint num_entries, cl_platform_id *platforms, cl_uint *num_platforms) {
    auto func = OpenCLSymbols::GetInstance()->clGetPlatformIDs;
    CHECK_NOTNULL(func);
...
cl_int clGetPlatformInfo(cl_platform_id platform, cl_platform_info param_name, size_t param_value_size,
    ...
    auto func = OpenCLSymbols::GetInstance()->clGetPlatformInfo;
    CHECK_NOTNULL(func);
...
cl_int clGetDeviceIDs(cl_platform_id platform, cl_device_type device_type, cl_uint num_entries, cl_device_id *devices,
                      cl_uint *num_devices) {
    auto func = OpenCLSymbols::GetInstance()->clGetDeviceIDs;
    CHECK_NOTNULL(func);
...
cl_int clGetDeviceInfo(cl_device_id device, cl_device_info param_name, size_t param_value_size, void *param_value,
    ...
    auto func = OpenCLSymbols::GetInstance()->clGetDeviceInfo;
    CHECK_NOTNULL(func);
...
cl_context clCreateContext(const cl_context_properties *properties, cl_uint num_devices, const cl_device_id *devices,
                           ...
                           cl_int *errcode_ret) {
    auto func = OpenCLSymbols::GetInstance()->clCreateContext;
    CHECK_NOTNULL(func);
...
cl_context clCreateContextFromType(const cl_context_properties *properties, cl_device_type device_type,
    ...
    auto func = OpenCLSymbols::GetInstance()->clCreateContextFromType;
    CHECK_NOTNULL(func);
...
cl_int clRetainContext(cl_context context) {
    auto func = OpenCLSymbols::GetInstance()->clRetainContext;
    CHECK_NOTNULL(func);
    return func(context);
}
...
cl_int clReleaseContext(cl_context context) {
    auto func = OpenCLSymbols::GetInstance()->clReleaseContext;
    CHECK_NOTNULL(func);
    return func(context);
}
...
cl_int clGetContextInfo(cl_context context, cl_context_info param_name, size_t param_value_size, void *param_value,
                        size_t *param_value_size_ret) {
    auto func = OpenCLSymbols::GetInstance()->clGetContextInfo;
    CHECK_NOTNULL(func);
...
cl_program clCreateProgramWithSource(cl_context context, cl_uint count, const char **strings, const size_t *lengths,
                                     cl_int *errcode_ret) {
    auto func = OpenCLSymbols::GetInstance()->clCreateProgramWithSource;
    CHECK_NOTNULL(func);
...
cl_int clGetProgramInfo(cl_program program, cl_program_info param_name, size_t param_value_size, void *param_value,
                        size_t *param_value_size_ret) {
    auto func = OpenCLSymbols::GetInstance()->clGetProgramInfo;
    CHECK_NOTNULL(func);
...
cl_int clGetProgramBuildInfo(cl_program program, cl_device_id device, cl_program_build_info param_name,
    ...
    auto func = OpenCLSymbols::GetInstance()->clGetProgramBuildInfo;
    CHECK_NOTNULL(func);
...
cl_int clRetainProgram(cl_program program) {
    auto func = OpenCLSymbols::GetInstance()->clRetainProgram;
    CHECK_NOTNULL(func);
    return func(program);
}
...
cl_int clReleaseProgram(cl_program program) {
    auto func = OpenCLSymbols::GetInstance()->clReleaseProgram;
    CHECK_NOTNULL(func);
    return func(program);
}
...
cl_int clBuildProgram(cl_program program, cl_uint num_devices, const cl_device_id *device_list, const char *options,
    ...
    auto func = OpenCLSymbols::GetInstance()->clBuildProgram;
    CHECK_NOTNULL(func);
...
cl_kernel clCreateKernel(cl_program program, const char *kernelName, cl_int *errcode_ret) {
    auto func = OpenCLSymbols::GetInstance()->clCreateKernel;
    CHECK_NOTNULL(func);
...
cl_int clRetainKernel(cl_kernel kernel) {
    auto func = OpenCLSymbols::GetInstance()->clRetainKernel;
    CHECK_NOTNULL(func);
    return func(kernel);
}
...
cl_int clReleaseKernel(cl_kernel kernel) {
    auto func = OpenCLSymbols::GetInstance()->clReleaseKernel;
    CHECK_NOTNULL(func);
    return func(kernel);
}
...
cl_int clSetKernelArg(cl_kernel kernel, cl_uint arg_index, size_t arg_size, const void *arg_value) {
    auto func = OpenCLSymbols::GetInstance()->clSetKernelArg;
    CHECK_NOTNULL(func);
...
cl_mem clCreateBuffer(cl_context context, cl_mem_flags flags, size_t size, void *host_ptr, cl_int *errcode_ret) {
    auto func = OpenCLSymbols::GetInstance()->clCreateBuffer;
    CHECK_NOTNULL(func);
...
cl_int clRetainMemObject(cl_mem memobj) {
    auto func = OpenCLSymbols::GetInstance()->clRetainMemObject;
    CHECK_NOTNULL(func);
    return func(memobj);
}
...
cl_int clReleaseMemObject(cl_mem memobj) {
    auto func = OpenCLSymbols::GetInstance()->clReleaseMemObject;
    CHECK_NOTNULL(func);
    return func(memobj);
}
...
cl_int clGetImageInfo(cl_mem image, cl_image_info param_name, size_t param_value_size, void *param_value,
                      size_t *param_value_size_ret) {
    auto func = OpenCLSymbols::GetInstance()->clGetImageInfo;
    CHECK_NOTNULL(func);
...
cl_int clRetainCommandQueue(cl_command_queue command_queue) {
    auto func = OpenCLSymbols::GetInstance()->clRetainCommandQueue;
    CHECK_NOTNULL(func);
...
cl_int clReleaseCommandQueue(cl_command_queue command_queue) {
    auto func = OpenCLSymbols::GetInstance()->clReleaseCommandQueue;
    CHECK_NOTNULL(func);
    return func(command_queue);
}
...
cl_int clEnqueueReadBuffer(cl_command_queue command_queue, cl_mem buffer, cl_bool blocking_read, size_t offset,
                           ...
                           cl_event *event) {
    auto func = OpenCLSymbols::GetInstance()->clEnqueueReadBuffer;
    CHECK_NOTNULL(func);
...
cl_int clEnqueueWriteBuffer(cl_command_queue command_queue, cl_mem buffer, cl_bool blocking_write, size_t offset,
    ...
    auto func = OpenCLSymbols::GetInstance()->clEnqueueWriteBuffer;
    CHECK_NOTNULL(func);
...
void *clEnqueueMapBuffer(cl_command_queue command_queue, cl_mem buffer, cl_bool blocking_map, cl_map_flags map_flags,
    ...
    auto func = OpenCLSymbols::GetInstance()->clEnqueueMapBuffer;
    CHECK_NOTNULL(func);
...
void *clEnqueueMapImage(cl_command_queue command_queue, cl_mem image, cl_bool blocking_map, cl_map_flags map_flags,
                        ...
                        cl_int *errcode_ret) {
    auto func = OpenCLSymbols::GetInstance()->clEnqueueMapImage;
    CHECK_NOTNULL(func);
...
cl_int clEnqueueUnmapMemObject(cl_command_queue command_queue, cl_mem memobj, void *mapped_ptr,
                               cl_uint num_events_in_wait_list, const cl_event *event_wait_list, cl_event *event) {
    auto func = OpenCLSymbols::GetInstance()->clEnqueueUnmapMemObject;
    CHECK_NOTNULL(func);
...
cl_int clGetKernelWorkGroupInfo(cl_kernel kernel, cl_device_id device, cl_kernel_work_group_info param_name,
                                size_t param_value_size, void *param_value, size_t *param_value_size_ret) {
    auto func = OpenCLSymbols::GetInstance()->clGetKernelWorkGroupInfo;
    CHECK_NOTNULL(func);
...
cl_int clGetEventProfilingInfo(cl_event event, cl_profiling_info param_name, size_t param_value_size, void *param_value,
                               size_t *param_value_size_ret) {
    auto func = OpenCLSymbols::GetInstance()->clGetEventProfilingInfo;
    CHECK_NOTNULL(func);
...
cl_int clEnqueueNDRangeKernel(cl_command_queue command_queue, cl_kernel kernel, cl_uint work_dim,
                              ...
                              const cl_event *event_wait_list, cl_event *event) {
    auto func = OpenCLSymbols::GetInstance()->clEnqueueNDRangeKernel;
    CHECK_NOTNULL(func);
...
cl_int clWaitForEvents(cl_uint num_events, const cl_event *event_list) {
    auto func = OpenCLSymbols::GetInstance()->clWaitForEvents;
    CHECK_NOTNULL(func);
...
cl_int clRetainEvent(cl_event event) {
    auto func = OpenCLSymbols::GetInstance()->clRetainEvent;
    CHECK_NOTNULL(func);
    return func(event);
}
...
cl_int clReleaseEvent(cl_event event) {
    auto func = OpenCLSymbols::GetInstance()->clReleaseEvent;
    CHECK_NOTNULL(func);
    return func(event);
}
...
cl_int clGetEventInfo(cl_event event, cl_event_info param_name, size_t param_value_size, void *param_value,
                      size_t *param_value_size_ret) {
    auto func = OpenCLSymbols::GetInstance()->clGetEventInfo;
    CHECK_NOTNULL(func);
    return func(event, param_name, param_value_size, param_value, param_value_size_ret);
}
...
cl_int clFlush(cl_command_queue command_queue) {
    auto func = OpenCLSymbols::GetInstance()->clFlush;
    CHECK_NOTNULL(func);
    return func(command_queue);
}
...
cl_int clFinish(cl_command_queue command_queue) {
    auto func = OpenCLSymbols::GetInstance()->clFinish;
    CHECK_NOTNULL(func);
    return func(command_queue);
}
...
cl_mem clCreateImage2D(cl_context context, cl_mem_flags flags, const cl_image_format *image_format, size_t imageWidth,
    ...
    auto func = OpenCLSymbols::GetInstance()->clCreateImage2D;
    CHECK_NOTNULL(func);
...
cl_mem clCreateImage3D(cl_context context, cl_mem_flags flags, const cl_image_format *image_format, size_t imageWidth,
    ...
    auto func = OpenCLSymbols::GetInstance()->clCreateImage3D;
    CHECK_NOTNULL(func);
...
cl_command_queue clCreateCommandQueue(cl_context context, cl_device_id device, cl_command_queue_properties properties,
                                      cl_int *errcode_ret) {
    auto func = OpenCLSymbols::GetInstance()->clCreateCommandQueue;
    CHECK_NOTNULL(func);
...
cl_int clGetCommandQueueInfo(cl_command_queue command_queue, cl_command_queue_info param_name, size_t param_value_size,
                             void *param_value, size_t *param_value_size_ret) {
    auto func = OpenCLSymbols::GetInstance()->clGetCommandQueueInfo;
    CHECK_NOTNULL(func);
...
cl_int clEnqueueCopyImage(cl_command_queue queue, cl_mem src_image, cl_mem dst_image, const size_t *src_origin,
                          ...
                          const cl_event *event_wait_list, cl_event *event) {
    auto func = OpenCLSymbols::GetInstance()->clEnqueueCopyImage;
    CHECK_NOTNULL(func);
...
cl_int clEnqueueCopyBufferToImage(cl_command_queue command_queue, cl_mem src_buffer, cl_mem dst_image,
                                  ...
                                  cl_uint num_events_in_wait_list, const cl_event *event_wait_list, cl_event *event) {
    auto func = OpenCLSymbols::GetInstance()->clEnqueueCopyBufferToImage;
    CHECK_NOTNULL(func);
...
cl_int clEnqueueCopyImageToBuffer(cl_command_queue command_queue, cl_mem src_image, cl_mem dst_buffer,
                                  ...
                                  cl_uint num_events_in_wait_list, const cl_event *event_wait_list, cl_event *event) {
    auto func = OpenCLSymbols::GetInstance()->clEnqueueCopyImageToBuffer;
    CHECK_NOTNULL(func);
...
cl_int clRetainDevice(cl_device_id device) {
    auto func = OpenCLSymbols::GetInstance()->clRetainDevice;
    CHECK_NOTNULL(func);
    return func(device);
}
...
cl_int clReleaseDevice(cl_device_id device) {
    auto func = OpenCLSymbols::GetInstance()->clReleaseDevice;
    CHECK_NOTNULL(func);
    return func(device);
}
...
cl_mem clCreateImage(cl_context context, cl_mem_flags flags, const cl_image_format *image_format,
    ...
    auto func = OpenCLSymbols::GetInstance()->clCreateImage;
    CHECK_NOTNULL(func);
...
cl_int clGetKernelSubGroupInfoKHR(cl_kernel kernel, cl_device_id device, cl_kernel_sub_group_info param_name,
                                  ...
                                  void *param_value, size_t *param_value_size_ret) {
    auto func = OpenCLSymbols::GetInstance()->clGetKernelSubGroupInfoKHR;
    CHECK_NOTNULL(func);
...
cl_command_queue clCreateCommandQueueWithProperties(cl_context context, cl_device_id device,
    ...
    auto func = OpenCLSymbols::GetInstance()->clCreateCommandQueueWithProperties;
    CHECK_NOTNULL(func);
    return func(context, device, properties, errcode_ret);
}
...
void *clGetExtensionFunctionAddress(const char *func_name) {
    auto func = OpenCLSymbols::GetInstance()->clGetExtensionFunctionAddress;
    CHECK_NOTNULL(func);
